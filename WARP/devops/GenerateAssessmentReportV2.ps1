...
function LoadDescriptionFile {
    ...
    elseif ($DevOpsCapability) {
        ...
        try {
            $descriptionsFile = Import-Csv "$workingDirectory\CAF Category DescriptionsV2.csv"
        }
        catch {
            Write-Error -Message "Unable to open $($workingDirectory)\CAF Category DescriptionsV2.csv"
            exit
        }
    }
...
if ($assessmentTypeCheck.contains("Cloud Adoption") ) {
    write-host "Detected Cloud Adoption Security Assessment from CSV title row"
    $CloudAdoption = $true
...
if ($WellArchitected) {
    ...
    if($CloudAdoption) {
        Write-host "Producing Cloud Adoption Security Assessment report..."
    } elseif($DevOpsCapability) {
